...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define APP_DHCP_STARTUP_TIMEOUT   (10U * NX_IP_PERIODIC_RATE)
#define APP_DHCP_RETRY_TIMEOUT      (3U * NX_IP_PERIODIC_RATE)
/* USER CODE END PD */
...
static void App_Configure_Dhcp_Gateway(void);
...
static VOID nx_app_thread_entry (ULONG thread_input)
{
  ...
  /*
   * Compromis demo : on attend le DHCP, mais pas indefiniment.
   * Sans IP, les capteurs et l'inference demarrent quand meme en mode offline.
   */
  if(tx_semaphore_get(&DHCPSemaphore, APP_DHCP_STARTUP_TIMEOUT) == TX_SUCCESS)
  {
    App_Configure_Dhcp_Gateway();
    PRINT_IP_ADDRESS(IpAddress);
  }
  else
  {
    printf("[NET] DHCP timeout, demarrage local sans IP\r\n");
  }
  ...
  /* Start the sensor / inference cycles even if the network is not ready. */
...
static VOID App_TCP_Thread_Entry(ULONG thread_input)
{
  ...
  ULONG last_gateway_ip = 0;
  UINT dns_resolved = NX_FALSE;
  ...
  /* Le thread peut demarrer avant le DHCP : il attendra une IP avant les POST. */
  ...
  while(1)
  {
    ...
    if ((nx_ip_address_get(&NetXDuoEthIpInstance, &current_ip, &current_mask) != NX_SUCCESS) ||
        (current_ip == NULL_ADDRESS))
    {
      strncpy((char*)g_post_status, "offline", sizeof(g_post_status) - 1);
      ((char*)g_post_status)[sizeof(g_post_status) - 1] = '\0';
      tx_thread_sleep(100);
      continue;
    }

    if (current_ip != last_gateway_ip)
    {
      App_Configure_Dhcp_Gateway();
      last_gateway_ip = current_ip;
      tx_thread_sleep(100);
    }

    /*
     * Resolution DNS optionnelle, faite seulement quand le reseau est pret.
     * Si 8.8.8.8 ne repond pas sur le reseau de demo, server_ip reste l'IP fixe.
     */
    if (dns_resolved == NX_FALSE)
    {
    ...
    if(status == NX_SUCCESS)
    {
      if(linkdown == 1)
      {
        ...
        if(status == NX_SUCCESS)
        {
          ...
          /* wait until an IP address is ready, without blocking this thread forever */
          if(tx_semaphore_get(&DHCPSemaphore, APP_DHCP_RETRY_TIMEOUT) != TX_SUCCESS)
          {
            printf("[NET] DHCP toujours indisponible, retry plus tard\r\n");
          }
          else
          {
            App_Configure_Dhcp_Gateway();
