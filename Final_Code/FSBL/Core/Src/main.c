...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
/* IWDG secure register — kick to prevent hardware watchdog reset */
#define IWDG_KR_S  (*(volatile uint32_t*)0x56004800UL)
/* USER CODE END 0 */
...
int main(void)
{
  ...
  /* USER CODE BEGIN 1 */
  /* Kick IWDG immediately — safe even if IWDG not started (write ignored) */
  IWDG_KR_S = 0xAAAAUL;
  /* USER CODE END 1 */
  ...
  IWDG_KR_S = 0xAAAAUL;
  MX_USART1_UART_Init();
  HAL_UART_Transmit(&huart1, (uint8_t*)"[DBG] GPIO+UART OK\r\n", 20, 1000);
  IWDG_KR_S = 0xAAAAUL;
  MX_ETH1_Init();
  HAL_UART_Transmit(&huart1, (uint8_t*)"[DBG] ETH OK\r\n", 14, 1000);
  IWDG_KR_S = 0xAAAAUL;
  MX_I2C1_Init();
  HAL_UART_Transmit(&huart1, (uint8_t*)"[DBG] I2C OK\r\n", 14, 1000);
  IWDG_KR_S = 0xAAAAUL;
  MX_CACHEAXI_Init();
  HAL_UART_Transmit(&huart1, (uint8_t*)"[DBG] CACHEAXI OK\r\n", 19, 1000);
  IWDG_KR_S = 0xAAAAUL;
  ...
  printf("\r\n");
  printf("╔══════════════════════════════════════╗\r\n");
  printf("║         AtmosAI — STM32N657X0        ║\r\n");
  printf("║  Capteur T/H/P + inference H+1 MLP  ║\r\n");
  printf("║  LED verte=Clair  rouge=Pluie        ║\r\n");
  printf("║  LED verte+rouge=Brouillard          ║\r\n");
  printf("╚══════════════════════════════════════╝\r\n");
  printf("\r\n");
  /* USER CODE END 2 */
...
void Error_Handler(void)
{
  ...
  while (1)
  {
    /* Kick IWDG so the chip stays alive and debugger can pause here */
    IWDG_KR_S = 0xAAAAUL;
