...
/* Stack statique pour le thread capteur — 4 KB pour laisser de la marge
 * aux appels h1_infer() (sinf/cosf/expf + 4 layers dense + ring buffer) */
...
UINT MX_NetXDuo_Init(VOID *memory_ptr)
{
  ...
  { extern UART_HandleTypeDef huart1;
    HAL_UART_Transmit(&huart1, (uint8_t*)"[3] MX_NetXDuo_Init\r\n", 21, 1000); }
  ...
  /* Le thread capteur utilise un stack statique dédié (s_sensor_stack)
   * afin de ne pas épuiser le byte pool et d'avoir assez de place
   * pour les appels ATON runtime + EmbedNets SW (~3 KB en pic). */
...
static VOID App_Sensor_Thread_Entry(ULONG thread_input)
{
    ...
    while(1)
    {
        ...
        /* Pousse dans le ring buffer (deltas historiques) */
        h1_push(g_temperature, g_humidity, g_pressure);

        /* ── Inférence H+1 — MLP embarqué (float32, Cortex-M55) ─── */
        ...
        if (res.ready) {
            ...
            printf("[NPU] H+1 : %s (%.1f%%)  C=%.2f P=%.2f B=%.2f\r\n",
