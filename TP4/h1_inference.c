...
H1Result h1_infer(void)
{
    ...
    /* Durée réelle d'historique disponible dans le buffer */
    ...
    if (k1h >= 0) {
        ...
        printf("[H1] !! delta 1h indisponible — historique insuffisant"
               " (pret dans ~%lu min %02lu s)\r\n",
    ...
    if (k3h >= 0) {
        ...
        uint32_t remain_s  = (elapsed_ms < need_ms) ? (need_ms - elapsed_ms) / 1000u : 0u;
        printf("[H1] !! delta 3h indisponible — historique insuffisant"
               " (pret dans ~%lu min %02lu s)\r\n",
               (unsigned long)(remain_s / 60),
               (unsigned long)(remain_s % 60));
    }
    ...
    /* ── Debug UART — features calculées ─────────────────── */
    printf("[H1] Deltas : dT1h=%+.2f  dT3h=%+.2f  dP1h=%+.2f  dP3h=%+.2f  dH1h=%+.2f\r\n",
           temp_d1h, temp_d3h, pres_d1h, pres_d3h, rhum_d1h);
    printf("[H1] Extras : dwpt_diff=%.2f  hsin=%.3f  hcos=%.3f\r\n",
           dwpt_diff, sinf(2.0f * 3.14159265f * (float)s_hour / 24.0f),
                      cosf(2.0f * 3.14159265f * (float)s_hour / 24.0f));
